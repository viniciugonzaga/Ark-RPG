{{-- resources/views/partials/rolagens-sistema.blade.php --}}
@props(['characters'])
...
<div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
    {{-- Coluna esquerda: Status e atributos --}}
    <div class="space-y-6">
        {{-- Seletor de ficha / modo manual --}}
            ...
            <div id="ficha-selector" class="mt-3">
                <select id="character-select" class="ark-input w-full">
                    ...
                    @foreach($characters as $char)
                        <option value="{{ $char->id }}" data-civilization="{{ strtolower($char->class_sub) }}">{{ strtoupper($char->name) }} (Nível {{ $char->level }})</option>
                    @endforeach
                </select>
            </div>
            <div id="manual-attributes" class="mt-3 hidden">
                ...
                <div class="grid grid-cols-5 gap-2">
                    @foreach(['for','agi','int','vig','set'] as $a)
                        <div>
                            <label class="text-[10px] uppercase block">{{ $a }}</label>
                            <input type="number" id="manual-{{ $a }}" value="1" min="0" max="10" class="ark-input w-full text-center text-sm">
                        </div>
                    @endforeach
                </div>
            </div>
    ...
    {{-- Coluna direita: Dados e Eventos --}}
    <div class="space-y-6">
        <div class="ark-panel p-6">
            ...
            <div class="space-y-5">
                ...
                <div class="grid grid-cols-6 gap-2">
                    @foreach([5,10,15,20,25,30] as $b)
                        <button onclick="setBonus({{ $b }})" class="bg-black/40 border border-cyan-400/20 hover:bg-cyan-500/30 text-cyan-200 text-xs font-bold p-2 rounded-lg transition-all hover:scale-105">+{{ $b }}</button>
                    @endforeach
                </div>
        ...
        <div class="ark-panel p-6">
            ...
            <div class="events-grid">
                @foreach(['sobrevivencia','efeito','item','traumas','epicos','joias','joias_raras','frutas'] as $tipo)
                <div class="event-block" data-event-type="{{ $tipo }}">
                    <img src="{{ asset("images/evento_{$tipo}_icon.png") }}" class="event-icon pulse-glow" style="cursor:pointer">
                    <div class="flex flex-col items-start gap-1"><span class="event-label">{{ ucfirst($tipo) }}</span><img src="{{ asset('images/rolar_evento_text.png') }}" class="event-text-img"></div>
                </div>
                <div id="event-result-{{ $tipo }}" class="event-result col-span-full"></div>
                @endforeach
            </div>
        </div>
    </div>
...
<script>
        ...
        for(let i=0; i<3; i++){
            ...
            div.innerHTML = `
                <div class="flex items-center gap-2">
                    <div class="attr-icon"><img src="{{ asset('images/dice_icon_atributo.png') }}" alt="dado" style="width:20px"></div>
    ...
    async function saveToDB(dice, event) {
        ...
        if (rollMode === 'ficha' && selectedCharId) {
            ...
            const fichas = {!! json_encode($characters->pluck('id')->toArray()) !!};
            if (fichas.length > 0) {
                charId = fichas[0];
            } else {
